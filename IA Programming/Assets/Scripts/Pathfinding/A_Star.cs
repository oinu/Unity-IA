...
public class A_Star : MonoBehaviour {
    ...
    void Start()
    {
        ...
        #region CREATION_TERRAIN & TARGET
        size = 10;

        //CREATE THE GRID & WEIGHT TERRAIN
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                ...
                //Associate the weight on terrains
        ...
        Vector3 p = area[randX, randZ].position;
        p.y = target.obj.transform.position.y;
        target.SetPosition(p);

        //ASSOCIATE THE NEIGHBORS & HEURISTIC COST
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                ...
                area[i, j].heuristicCost = Mathf.FloorToInt(Mathf.Abs(area[i, j].position.x - target.position.x)
                    + Mathf.Abs(area[i, j].position.z - target.position.z));
            }
        }
        #endregion
        ...
        p = area[size / 2, size / 2].GetPosition();
        p.y = 0.8f;
        pj.SetPosition(p);
    ...
    private void AStarBubbleSort()
    {
        ...
        for (int i = 0; i < fronter.Count; i++)
        {
            ...
            indexList[i, 0] = fronter[i].acomulatedCost + fronter[i].heuristicCost;
            indexList[i, 1] = i;
